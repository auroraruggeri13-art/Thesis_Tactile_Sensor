...
def patch_processor(processor, green_lower, green_upper,
                    exclude_lower=None, exclude_upper=None):
    """
    Monkey-patch a FinRayProcessor instance to use the interactively
    selected HSV bounds for every subsequent call to segment_green_gripper().
    """
    processor.green_lower   = green_lower
    processor.green_upper   = green_upper
    ...
    def _segment_with_hsv_patched(self, image):
        """
        HSV segmentation using interactively picked bounds.
          Stage 1 — bilateral filter + HSV threshold + convex hull → arm ROI.
          Stage 2 — fine HSV clipped to arm ROI.
          Stage 3 — exclude-colour mask subtracted.
          Stage 4 — fill tiny holes; keep rib gaps.
        """
        ...
        # Coarse arm ROI via convex hull
        ...
        if (self.exclude_lower is not None and self.exclude_upper is not None):
            exc_mask = cv2.inRange(hsv, self.exclude_lower, self.exclude_upper)
            # Dilate slightly so we remove fringe pixels too
            exc_mask = cv2.dilate(exc_mask, np.ones((5, 5), np.uint8), iterations=1)
            fine = cv2.bitwise_and(fine, cv2.bitwise_not(exc_mask))
    ...
    print(f"    Include: {green_lower} → {green_upper}")
