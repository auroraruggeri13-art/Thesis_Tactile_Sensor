...
def plot_time_series(df: pd.DataFrame, out_path: Path):
    ...
    line1, = ax1.plot(df["t"], df["pressure"], label="Pressure", linewidth=2)

    ax2 = ax1.twinx()
    ax2.set_ylabel("Fz [N]", fontsize=10)
    line2, = ax2.plot(df["t"], -df["fz"], label="Fz", linewidth=2.0, alpha=0.8, color='orange')
...
def plot_hysteresis(df: pd.DataFrame, out_path: Path, material: str = MATERIAL, 
    ...
    # Define colors for different cycles
    colors_load = plt.cm.Blues(np.linspace(0.4, 0.9, len(hyst_results)))
    colors_unload = plt.cm.Reds(np.linspace(0.4, 0.9, len(hyst_results)))
    ...
    for i, hyst_data in enumerate(hyst_results):
        ...
        ax.plot(loading['force'], loading['pressure'], '-', 
                color=colors_load[i], linewidth=2, 
                label=f'Loading {i+1}', alpha=0.7)
        ax.plot(unloading['force'], unloading['pressure'], '-', 
                color=colors_unload[i], linewidth=2, 
                label=f'Unloading {i+1}', alpha=0.7)
